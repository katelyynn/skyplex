...
execute as @a[distance=..1.25] if score @s housing.index = temporary_index housing.index run tag @s add housing.temporary_entry

# is owner nearby?
## if so, teleport them to house
execute as @a[tag=housing.temporary_entry,distance=..1.25] at @s run function sp:system/housing/enter/owner

# are normal players nearby?
## check config first
## if valid, teleport them to house
execute if score @e[tag=housing.location,tag=housing.temporary_location,limit=1] housing.config.open_to_visitors matches 1.. as @a[tag=!housing.temporary_entry,distance=..1.25] at @s run function sp:system/housing/enter/player
